// ****************************************************************************
//
//  $Id: cisstAlgorithmDirICP_vMFG_Mesh_MexInterface.cpp, v0.1 Exp $
//
//  Description:
//	        
//
...
#include "cisstAlgorithmDirICP_vMFG_Mesh_MexInterface.h"
...
{ // create new class instance
  ...
  plhs[0] = convertPtr2Mat < cisstAlgorithmDirICP_vMFG_Mesh_MexInterface >
    (new cisstAlgorithmDirICP_vMFG_Mesh_MexInterface);
}
...
{ // delete class instance
  ...
  destroyObject<cisstAlgorithmDirICP_vMFG_Mesh_MexInterface>(prhs[1]);
}
...
void CommandInitialize(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
  ...
  cisstAlgorithmDirICP_vMFG_Mesh_MexInterface& obj =
    *convertMat2Ptr<cisstAlgorithmDirICP_vMFG_Mesh_MexInterface>(prhs[1]);
  ...
  MEX_DEBUG(" ...position variance (sigma2_init)\n");
  if (mxGetN(prhs[7]) > 0)
  {
    ParseDouble(prhs[7], sigma2_init);
  }
  MEX_DEBUG(" ...orientation concentration (kinit)\n");
  if (mxGetN(prhs[8]) > 0)
  {
    ParseDouble(prhs[8], kinit);
  }
  MEX_DEBUG(" ...orientation concentration (wRpos)\n");
  if (mxGetN(prhs[9]) > 0)
  {
    ParseDouble(prhs[9], wRpos);
  }
  MEX_DEBUG(" ...dynamic parameter estimation (bDynamicParamEst)\n");
  if (mxGetN(prhs[10]) > 0)
  {
    ParseLogical(prhs[10], bDynamicParamEst);
  }
  ...
  cisstDirCovTree_Mesh *pCovTree = new cisstDirCovTree_Mesh(*pMesh, nThresh, diagThresh);
  ss.str("");
  ss << "Tree built:  NNodes = " << pCovTree->NumNodes() << "  NDatums = "
    << pCovTree->NumData() << "  TreeDepth = " << pCovTree->TreeDepth() << "\n";
  MEX_DEBUG(ss.str());
  ...
  double kfactor = 1.0;
  obj.pAlg = new cisstAlgorithmDirICP_vMFG_Mesh(
    pCovTree, samplePts, sampleNorms, kinit, sigma2_init, wRpos, kfactor, bDynamicParamEst);
}
...
void CommandSetSamples(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
  ...
  //  sigma2_init ~ variance of points        (double)      optional
  //  kinit ~ concentration of orientations   (double)      optional
  //  wRpos ~ weight of positions in estimating k (double)  optional
  //  bDynamicParamEst                        (bool)        optional
  //  
  ...
  if (mxGetN(prhs[4]) > 0)
  {
    ParseDouble(prhs[4], sigma2_init);
  }
  MEX_DEBUG(" ...orientation concentration (kinit)\n");
  if (mxGetN(prhs[5]) > 0)
  {
    ParseDouble(prhs[5], kinit);
  }
  MEX_DEBUG(" ...orientation concentration (wRpos)\n");
  if (mxGetN(prhs[6]) > 0)
  {
    ParseDouble(prhs[6], wRpos);
  }
  MEX_DEBUG(" ...dynamic parameter estimation (bDynamicParamEst)\n");
  if (mxGetN(prhs[7]) > 0)
  {
    ParseLogical(prhs[7], bDynamicParamEst);
  }
  ...
  obj.pAlg->SetNoiseModel(kinit, sigma2_init, wRpos, bDynamicParamEst);
}
...
void CommandICP_ComputeMatches(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
  ...
  cisstAlgorithmDirICP_vMFG_Mesh& alg = *obj.pAlg;
...
void CommandICP_RegisterMatches(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
  ...
  // Check parameters
  if (nlhs < 1 || nlhs > 3 || nrhs != 2)
  {
    MEX_ERROR("ComputeMatches: requires 1-3 outputs, 2 inputs.");
  }
  ...
  if (nlhs > 1)
  {
    plhs[1] = mxCreateDoubleScalar(alg.sigma2);
    plhs[2] = mxCreateDoubleScalar(alg.k);
  }
...
//    cisstAlgorithmDirICP_StdICP_Mesh* pAlg =
//        convertMat2Ptr<cisstAlgorithmDirICP_StdICP_Mesh>(prhs[1]);
//
//    // Check parameters
//    if (nlhs != 0 || nrhs != 2)
//    {
