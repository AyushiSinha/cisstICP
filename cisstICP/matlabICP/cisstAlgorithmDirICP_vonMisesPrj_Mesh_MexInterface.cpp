// ****************************************************************************
//
//  $Id: cisstAlgorithmDirICP_vonMisesPrj_Mesh_MexInterface.cpp, v0.1 Exp $
//
//  Description:
//	        
//
...
#include "cisstAlgorithmDirICP_vonMisesPrj_Mesh_MexInterface.h"
...
{ // create new class instance
  ...
  plhs[0] = convertPtr2Mat < cisstAlgorithmDirICP_vonMisesPrj_Mesh_MexInterface >
    (new cisstAlgorithmDirICP_vonMisesPrj_Mesh_MexInterface);
}
...
{ // delete class instance
  ...
  destroyObject<cisstAlgorithmDirICP_vonMisesPrj_Mesh_MexInterface>(prhs[1]);
}
...
void CommandInitialize(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
  ...
  cisstAlgorithmDirICP_vonMisesPrj_Mesh_MexInterface& obj =
    *convertMat2Ptr<cisstAlgorithmDirICP_vonMisesPrj_Mesh_MexInterface>(prhs[1]);

  // Check parameters
  if (nlhs != 0 || nrhs < 10)
  {
    MEX_ERROR("Initialize: requires 0 outputs, 10 inputs.");
  }
  ...
  vctDynamicVector<vct2> sampleNorms2d;
  vctDynamicVector<vctRot3> Rx_plane;
  ...
  if (mxGetN(prhs[6]) > 0)
  {
    Parse2DVectorArray_Double(prhs[6], sampleNorms2d);
  }
  MEX_DEBUG(" ...Rx_plane\n");
  if (mxGetN(prhs[7]) > 0)
  {
    ParseRotations(prhs[7], Rx_plane);
  }
  ...
  if (mxGetN(prhs[9]) > 0)
  {
    ParseMatrixArray3x3_Double(prhs[9], M);
  }
  ...
  obj.pAlg = new cisstAlgorithmDirICP_vonMisesPrj_Mesh(
    pCovTree, samplePts, sampleNorms2d,
    Rx_plane, k, M);
}
...
void CommandSetSamples(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
  ...
  if (nlhs != 0 || nrhs != 7)
  {
    MEX_ERROR("SetSamples: requires 0 outputs, 7 inputs.");
  }
  ...
  MEX_DEBUG(" ...SampleNorms2d\n");
  Parse2DVectorArray_Double(prhs[3], sampleNorms2d);
  MEX_DEBUG(" ...Rx_plane\n");
  ParseRotations(prhs[4], Rx_plane);
  MEX_DEBUG(" ...sample orientation concentrations (k)\n");
  ParseVector_Double(prhs[5], k);
  MEX_DEBUG(" ...sample covariances (M)\n");
  ParseMatrixArray3x3_Double(prhs[6], M);

  MEX_DEBUG(" ...done\n");

  // set samples
  obj.pAlg->SetSamples(samplePts, sampleNorms2d, Rx_plane, k, M);
}
...
void CommandICP_ComputeMatches(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
  ...
  cisstAlgorithmDirICP_vonMisesPrj_Mesh& alg = *obj.pAlg;
